...
N.B: 
- The `--services` option exposes the same HTTP API as the `--listen` option, but without the `/connect` endpoint. This is useful for scenarios where the `gvforwarder`/`vm` tool is not run on the guest but you still want to expose services and stats endpoints.
- The gateway (http://192.168.127.1:80 accessible from within the VM) only exposes port forwarding endpoints (`/services/forwarder/*`), not the full API. This minimizes the attack surface for potentially untrusted containers.

#### API Authentication

The gateway API endpoint (accessible from the VM at http://192.168.127.1:80) can be secured with Bearer token authentication. When enabled, all port forwarding requests from within the VM require an `Authorization` header with the token.

**Note:** Authentication only applies to the gateway endpoint accessible from the VM. Host-side endpoints (`--listen` or `--services`) remain unauthenticated as they are already protected by unix socket file permissions or localhost binding.

**Token Format:**
The token is a base64url-encoded random string, typically 44 characters long (minimum of 32 characters):
```
...
chmod 0600 /tmp/gvproxy-token

# Start gvproxy with authentication
gvproxy --api-token-file /tmp/gvproxy-token --listen unix:///tmp/network.sock
```

Option 2 - Using environment variable:
```bash
# Set token via environment variable
export GVISOR_API_TOKEN="xK8vN2Qp_7RmLwE4sJ9nY3Tc6Vh5Gz1Ua8Fb0Pd2Xe4="
...
curl -H "Authorization: Bearer $TOKEN" \
  http://192.168.127.1/services/forwarder/all
```

**Host-side requests** (via `--listen` or `--services` sockets) do not require authentication.

**Token File Format:**
The token file should contain only the token string (whitespace is automatically trimmed):
```
xK8vN2Qp_7RmLwE4sJ9nY3Tc6Vh5Gz1Ua8Fb0Pd2Xe4=
```

**Backwards Compatibility:**
If neither `--api-token-file` nor `GVISOR_API_TOKEN` is set, authentication is disabled and the API accepts all requests without tokens.

For complete details, see [API_TOKEN_AUTHENTICATION.md](API_TOKEN_AUTHENTICATION.md).
...
- **Gateway** (http://192.168.127.1:80 from VM): Authentication required when enabled via `--api-token-file` or `GVISOR_API_TOKEN`
...
With authentication enabled (from within the VM):
```
$ TOKEN="xK8vN2Qp_7RmLwE4sJ9nY3Tc6Vh5Gz1Ua8Fb0Pd2Xe4="
$ curl -H "Authorization: Bearer $TOKEN" \
  http://192.168.127.1/services/forwarder/expose \
  -X POST -d '{"local":":6443","remote":"192.168.127.2:6443"}'
```
